...
description: Explains the TencentDB-Agent-Memory four-layer pyramid (L0 Conversation → L1 Atom → L2 Scene → L3 Persona) and hybrid recall. Use when the user asks how memory is structured, what L0/L1/L2/L3 mean, where persona.md / scene blocks / index.db live, how recall works, or what gets injected into each turn.
---
...
| **L1 Atom** | Atomic facts extracted by agent | FTS5 SQLite (`index.db`) + vector (`vectors.db`) + JSONL shards | Per-fact recall. |
...
When `UserPromptSubmit` fires, the hook runs hybrid recall and injects `<memory-context>` via `additionalContext`:

- **L3 persona** (stable profile, always included — top 5 attributes)
- **L1 recalled memories** (top-K from hybrid FTS5 + vector search, merged via RRF)
- Total budget: ~280 tokens max

## Extraction pipeline

Memory extraction is agent-driven, not LLM-API-driven:

1. **Auto-capture** (`Stop` hook): each turn's user text is stored as a raw capture in FTS5 + vector for immediate recall.
2. **memory-seed skill**: Claude agent reads past L0 transcripts and extracts structured L1 atoms (persona, episodic, instruction types).
3. **memory-consolidate skill**: Claude agent groups L1 atoms into L2 scenes and synthesizes L3 persona.
...
│       ├── l1/
│       └── scenes/
├── models/                    # EmbeddingGemma GGUF model cache
...
- **Stop** → auto-capture latest turn to FTS5 + vector + consolidation check (asyncRewake)
